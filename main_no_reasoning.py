...
def main():
    ...
    qs = functions_no_reasoning.load_CRT(file)

    print("{} CRT task (with no explicit instruction to reason) is being performed by GPT-2 {} model. It will take a while.".format(crt_test, language_m))

    tracker.start_task("{}_CRT_using_GPT-2_no_reasoning{}".format(crt_test, language_m))

    # TASK
    final_responses = functions_no_reasoning.CRT(response_generator, qs)

    tracker.stop_task("{}_CRT_using_GPT-2_no_reasoning{}".format(crt_test, language_m))

    print("Saving the output...")

    functions_no_reasoning.save_output("{}_CRT_GPT-2_{}".format(crt_test, language_m), final_responses)
